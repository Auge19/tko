...
function valueToChangeAddItem(value, index): ChangeAddItem {
  return { status: 'added', value: value, index: index, isBatch: false }
}
...
export class ForEachBinding extends AsyncBindingHandler {
  ...
  onArrayChange(changeSet, isInitial) {
    var changeMap: ChangeMap = { added: [], deleted: [] }
  ...
  removeNodes(nodes) {
    ...
    if (this.beforeRemove) {
      var beforeRemoveReturn = this.beforeRemove({ nodesToRemove: nodes, foreachInstance: this }) || {}
