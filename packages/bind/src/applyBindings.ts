...
import { dependencyDetection, type Observable } from '@tko/observable'
...
function getBindingProvider(): any {
  // TODO
  ...
  if (sourceBindings && typeof sourceBindings !== 'function') {
    ...
    if (isProviderForNode(provider, node)) {
      ...
      var bindingsUpdater: any = computed(
        function () {
          bindings = sourceBindings
            ? (sourceBindings as any)(bindingContext, node)
...
function getBindingContext<T = any>(
  viewModelOrBindingContext?: MaybeBindingContextOrObservable<T>,
...
export function applyBindingAccessorsToNode<T = any>(
  ...
  return applyBindingsToNodeInternal<T>(
    node,
    bindings,
    getBindingContext<T>(viewModelOrBindingContext),
    asyncBindingsApplied
  )
}
